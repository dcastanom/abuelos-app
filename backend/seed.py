"""
Seed script — clears all tables and inserts fresh demo data.
...
from app.db.session import close_db, connect_db, get_session_factory
...
async def seed() -> None:
    ...
    async with session_factory() as db:
        # ── Wipe (child → parent, FK-safe) ──────────────────────────────────────
        await db.execute(delete(NursingNote))
        await db.execute(delete(Resident))
    ...
    print("  Admin        : admin@sanjose.com  /  Admin123!")
    print("  Enfermera    : enfermera@sanjose.com  /  Nurse123!")
    print("  Doctor       : doctor@sanjose.com  /  Doctor123!")
